...
$R_{\max}$      & MTPD (DRTO 상한)         & $24.0$\,h $= 4R_0$, 최대허용중단기간 \\
...
$\eta$          & 효율비 (Efficiency ratio) & $\DRTO / R_0 \in (0,\,4)$ \\
$\bar{\eta}$    & 효율비 평균              & 각 조건별 $\eta$ 표본 평균 \\
$\text{SD}(\eta)$ & 효율비 표준편차         & 각 조건별 $\eta$ 표본 표준편차 \\
\midrule
...
\jrev{H8}              & L4 이중채널 검증         & Tail/Core $k_O$ 감쇠비 $< 1$ (P85.8 분할) \\
...
\midrule
\endhead
동적 복구목표시간 & Dynamic RTO & 관측성·불확실성을 반영한 동적 RTO \\
...
이중채널 감쇠 & Dual-channel Attenuation & 극단적 불확실성(Tail 영역)에서 불확실성 채널이 지배적으로 작용하여 관측성 채널의 복구 단축 효과가 구조적으로 약화되는 현상 ($k_O$ 감쇠비 $0.52\times$) \\
...
효율비 & Efficiency Ratio ($\eta$) & $\DRTO / R_0$, 기준 대비 비율 \\
...
\midrule
{BCM} & {Business Continuity Management} & {사업연속성 관리. 중단 상황에서 핵심 업무를 유지·복구하기 위한 관리 체계} \\
{BCP} & {Business Continuity Plan} & {사업연속성 계획. 업무 중단 시 복구 절차를 문서화한 계획서} \\
{BCMS} & {Business Continuity Management System} & {사업연속성 관리 체계. \nrev{ISO 22301:2019}에 따른 BCM 구현·운영 통합 시스템} \\
...
\midrule
{\nrev{ISO 22301:2019}} & {ISO 22301:2019} & {사업연속성 관리체계 국제표준. BIA, RTO, 복구 전략 등을 규정} \\
...
{STIX} & {Structured Threat Information Expression} & {사이버 위협 정보 표준 교환 형식. TAXII와 함께 위협 인텔리전스 자동 연동} \\
...
9 & 외부 공급업체 장애 시 대체 방안(plan~B)이 수립되어 있는가? & $U$ \\
...
{$\DRTO$ 모델의 산업별 적용 가능성과 효율비($\eta$) 기반 활용 체계를 평가하는

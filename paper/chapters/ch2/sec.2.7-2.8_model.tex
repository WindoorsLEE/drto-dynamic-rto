...
앞서 설명한 이론적 배경과 선행 연구의 한계를 토대로, 동적
...
Lee and Cheung(2026)에서 수립한 관측성 기반 모델(C0$\to$C1)과
...
\chg{모델의 기저 함수는} 수정 시그모이드(modified sigmoid)로서, \m{식~(\ref{eq:sigmoid})에 나타낸 바와 같이}
...
\medskip
두 채널의 바운딩 수식은 \m{식~(\ref{eq:E_O_bnd}) 및 (\ref{eq:E_U_bnd})에 나타낸 바와 같이} 대칭적 구조를 갖는다.
\begin{align}
  ...
  E_{U,\text{bnd}} &= \;(R_{\max} - R_0) \cdot S(z_U),
    ...
    \label{eq:E_U_bnd}
\end{align}
시그모이드 입력 인자 $z_O$, $z_U$는 \m{식~(\ref{eq:z_O}) 및 (\ref{eq:z_U})에 나타낸 바와 같이} 각각
\begin{align}
...
작동한다. 둘째는 물리적 범위 분리이다. 관측성의 span은 $R_0$이므로 복구 시간을 기준선 아래로 작용하도록 하고, 불확실성의 span은 $R_{\max} - R_0$이므로 기준선 위로 작용하도록 하여 각각의 두 채널의 방향과 범위를 분리한다. 셋째는 상보적 가중이다. 관측성
...
효과 방향     & \m{하향($\DRTO$ 감소)}                 & \m{상향($\DRTO$ 증가)} \\
\bottomrule
\end{tabular}
\end{table}

\chg{최종 $\DRTO$는} 기준선 $R_0$에 개별 바운딩된 두 효과를 가산하여 식 (2.10)을 통해 산출한다.
...
$\DRTO$는 \m{식~(\ref{eq:bounds})에 나타낸 바와 같이} 항상 다음의 경계 내에 위치한다.
...
\chg{조건 간 비교를 위해} 무차원 정규화 비율 $\eta$를 \m{식~(\ref{eq:eta})에 나타낸 바와 같이} 정의한다.
...
\chg{그림에서 관측성 경로는 기준선 기준으로 상단에, 불확실성 경로는 기준선 하단에 표시되며, 두 경로는 각각 독립적인 span에서 시그모이드 바운딩을 거쳐 가산 노드에서 기준선 $R_0$와 합산된다. 곡률 매개변수 $\kappa = 1.2$는 두 경로에 공통으로 적용된다.}
...
각 매개변수의 설정 근거는 Lee and Cheung(2026), 국제 표준, 또는 수학적 제약 조건에 기반한다.
...
$R_{\max}$ & DRTO \m{상한(MTPD)} & 24.0 & h & $R_{\max} = 4 \cdot R_0$ (MTPD 기반 상한) \\
...
$0 < \DRTO < R_{\max}$가 $\kappa$에 무관하게 성립한다. 이로써 (명제 2.1) 증명이 완료되었다.
...
\chg{이처럼 C1에서는 관측성 활성화가 복구 시간을 단축하는 방향으로만 작용하며, 그 평균적 단축 효과와 효과 크기가 기준선(C0) 대비 통계적으로 유의한가는 본 장이 제시한 이론적 예측에 해당한다. 표본 기반 실증값(평균 효율성 비율, 단축률, Cohen's $d$)은 4.1(C0와 C1의 관측성 효과)에서 정량적으로 검증한다.}
...
비교(동일 기댓값 $3.0$ 설계)가 훼손된다.}

가우시안 분포는 대칭적이고 경꼬리(light-tail) 특성을 가지므로,
C2 조건은 일상적 운영 변동, \chg{곧 인력 가용성의 일시적 변화와
장비 성능의 통상적 편차, 절차 수행 시간의 자연적 변이를} 반영한다.

\chg{C2에서는 이러한 가우시안 불확실성의 결합이 복구 시간을 연장하는 방향으로 작용하리라 예측되며, 그 평균적 연장 크기와 C1 대비 효과 크기는 4.2(C2 가우시안 불확실성 결과)에서 정량적으로 검증한다.}
...
\chg{파레토 분포는 중꼬리 특성을 가지므로, C3는 평균 수준에서는 C2와 유사하더라도 극단 백분위로 갈수록 $\DRTO$가 C2를 크게 상회하리라 예측된다. 두 분포의 누적분포함수가 교차하는 지점이 존재하며, 본 연구에서는 이를 ``CDF 교차점(crossover)''으로 명명한다. 교차점을 경계로 핵심 영역(Core)과 꼬리 영역(Tail)을 분할하면, 꼬리 영역에서는 시그모이드 포화로 인해 관측성 가중치 $k_O$의 회귀 계수가 감쇠하는 이중채널 비선형 상호작용(dual-channel nonlinear interaction)이 나타나리라 예측된다. 교차점의 구체적 위치와 조건별 평균 효율성 비율, 그리고 감쇠의 실증값은 4.10(이중채널 감쇠 메커니즘)에서 상세히 분석한다.}

...
\chg{탐색은 표본 크기 $n = 10{,}000$, seed $= 42$에서 수행하였으며, 표의 \erf{$R^2_{(2\mathrm{q})}$는 2차 다항 회귀의}{$R^2$는 $\eta$에 대한 $k_O\!\cdot\!O$ 단순회귀의} 결정계수이다.}
...
$\kappa$ & 평균 $\bar{\eta}_{\text{C1}}$ & 단축률 (\%) & $|d|_{\text{C1vsC0}}$
  & $R^2_{\erf{(2\mathrm{q})}{(k_O\cdot O)}}$ & 포화율 (\%) & $F(\kappa)$ \\
\midrule
0.2  & 0.975 &  2.5 & \erf{1.150}{1.149} & 1.000 &  0.0 & \erf{0.813}{0.459} \\
0.5  & 0.937 &  6.3 & \erf{1.150}{1.153} & 1.000 &  0.0 & \erf{0.855}{0.684} \\
1.0  & 0.876 & 12.4 & \erf{1.170}{1.171} & 0.999 &  0.0 & \erf{0.957}{0.967} \\
\rowcolor{green!10}
1.2  & 0.853 & 14.7 & 1.180 & 0.999 &  0.0 & \textbf{1.000} \\
1.5  & 0.818 & 18.2 & \erf{1.200}{1.197} & 0.997 &  0.0 & \erf{0.947}{0.951} \\
2.0  & 0.765 & 23.5 & \erf{1.230}{1.231} & 0.993 &  0.0 & \erf{0.859}{0.697} \\
3.0  & 0.673 & 32.7 & \erf{1.310}{1.312} & 0.975 &  0.0 & \erf{0.858}{0.209} \\
5.0  & 0.538 & 46.2 & \erf{1.490}{1.491} & 0.908 &  3.9 & \erf{0.566}{0.007} \\
\bottomrule
\end{tabularx}
\end{table}

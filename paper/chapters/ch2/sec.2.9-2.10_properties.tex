...
\chg{앞서 2.5절에서 바운딩 기법의 관점에서 개관한 기본 성질을, 본 절에서는 $\DRTO$ 바운딩 함수로서 2계 도함수와 변곡점, 곡률 거동까지 포함하여 엄밀하게 정리한다.}
...
$0$ 또는 $R_{\max}$에 도달하는 것은 불가능하다. 넷째, 원점 대칭성(홀함수)으로서
...
%% -------------------------------------------------------------------
\subsection{소 $\kappa \cdot z$ 영역에서의 선형 근사}
\label{subsec:linear-approx}
%% -------------------------------------------------------------------

$|z|$가 충분히 작을 때, $S(z)$는 1차 테일러 전개로 근사된다.
...
본 연구는 선행 연구\citep{lee2026drto}의 다기준 최적화(multi-criteria
...
\chg{첫째는 효과 크기(Effect Size)로서 C1 대 C0의 Cohen's $d$ 절대값
$|d_{\text{C1vsC0}}|$이며, $\kappa$가 클수록 관측성의 효과가 증가하여 $|d|$가 커지므로
$|d| \geq 0.8$(대효과)을 기준으로 한다. 둘째는 포화율(Saturation Rate)로서 관측성
채널에서 $|S(z_O)| \geq 0.99$인 표본의 비율이며, $\kappa$가 너무 크면 다수의 표본이 포화
...
넷째는 RTO 단축률(Reduction Rate)로서 C1 평균 효율비에 의한 단축률
...
4개 기준을 종합하는 종합점수(composite score) $F(\kappa)$를
곱함수(product function)로 정의한다.
...
\chg{첫째, 실무 유의성을 나타내는 $f_1$은} 평균 감소율 $\bar{r}(\kappa) = (1 - \bar{\eta}) \times 100$이
...
\chg{둘째, 비포화를 나타내는 $f_2$는} 포화율(시그모이드 출력 $> 0.95$인 비율)이 5\% 미만이면 1, 아니면 0이다.
    \begin{equation}
...
\chg{셋째, 효과 크기를 나타내는 $f_3$는} C0 대비 Cohen's $d$의 절대값이 $\geq 0.8$이면 1이다.
    \begin{equation}
      ...
      \label{eq:f3}
    \end{equation}
    여기서 C0 조건은 $\eta \equiv 1.000$의 결정론적 상수이므로 합동 표준편차 공식의 분모가
    인위적으로 축소되는 것을 피하기 위해 C1 자체의 표준편차를 분모로 사용하는 일표본 효과
    크기를 적용한다. $\bar{\eta}_{\text{C1}} = 0.853$, $s_{\text{C1}} = 0.125$이므로
    $d = (0.853 - 1.000)/0.125 = -1.176 \approx -1.180$이고, $|d| \geq 0.8$이므로 $f_3 = 1.0$이다
    (Cohen's $d$의 정의와 해석 기준은 \secref{sec:ch5_effect_size} 참조).

\chg{넷째, 설명력을 나타내는 $f_4$는} $\eta_{C1}$에 대한 회귀 $R^2$이 $\geq 0.95$이면 1이다.
    \begin{equation}
...
\p{[}표~\ref{tab:kappa-grid}\p{]}에서 확인되듯이, $\kappa = 1.2$에서 종합점수
...
\begin{table}[htbp]
\centering
\caption{$\kappa$ 격자 탐색 결과 ($n = 10{,}000$, seed $= 42$)}
...
\midrule
0.2  & 0.975 &  2.5 & 1.150 & 1.000 &  0.0 & 0.813 \\
0.5  & 0.937 &  6.3 & 1.150 & 1.000 &  0.0 & 0.855 \\
1.0  & 0.876 & 12.4 & 1.170 & 0.999 &  0.0 & 0.957 \\
\rowcolor{green!10}
1.2  & 0.853 & 14.7 & 1.180 & 0.999 &  0.0 & \textbf{1.000} \\
1.5  & 0.818 & 18.2 & 1.200 & 0.997 &  0.0 & 0.947 \\
2.0  & 0.765 & 23.5 & 1.230 & 0.993 &  0.0 & 0.859 \\
3.0  & 0.673 & 32.7 & 1.310 & 0.975 &  0.0 & 0.858 \\
5.0  & 0.538 & 46.2 & 1.490 & 0.908 &  3.9 & 0.566 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{axis}[
  ...
  \addplot[thick, black, smooth, mark=*, mark size=2pt]
    coordinates {
      (0.2, 0.813) (0.5, 0.855) (1.0, 0.957) (1.2, 1.000)
      (1.5, 0.947) (2.0, 0.859) (3.0, 0.858) (5.0, 0.566)
    };

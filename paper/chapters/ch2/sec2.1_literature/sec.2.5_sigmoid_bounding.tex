...
본 연구에서 채택한 수정 시그모이드(modified sigmoid) 함수는 \m{식~(\ref{eq:sigmoid-def})에 나타낸 바와 같이} 정의된다.
\begin{equation}
...
성질의 엄밀한 정리와 2계 도함수와 변곡점 분석은 모델의 수학적 성질을 다루는 2.9(시그모이드 바운딩의 수학적 성질)에서 종합한다.}
...
설계 요구조건을 모두 충족하는 것은 세 후보 중 시그모이드 바운딩뿐임을 알 수 있다.}

[표 2-7]에서는 바운딩의 세 가지 기법의 특성을 종합적으로 비교하였다.
...
\chg{\p{[}표~\ref{tab:bounding-comparison}\p{]}에서 선형 모델은 경계를 초과할 수 있고 클리핑은 경계점에서 불연속인 반면, 시그모이드는 $(-1, +1)$ 범위 안에서 경계에 매끄럽게 점근하면서도 전 구간에서 기울기가 양으로 유지되는 점근적 바운딩을 제공하고 있다.}
시그모이드 바운딩은 경계 영역에서의 정보 보존성, 미분 가능성,
운영 해석 가능성 측면에서 $\DRTO$ 프레임워크에 가장 적합함을 알 수 있다.
\figref{fig:bounding-comparison}\refpo{fig:bounding-comparison}{은}{는} 세 바운딩 기법을 시각적으로 비교하였다.
...
불확실성 채널이 지배적으로 작용하면서 관측성 채널의 단축 효과가 구조적으로 점진적 감쇠되는

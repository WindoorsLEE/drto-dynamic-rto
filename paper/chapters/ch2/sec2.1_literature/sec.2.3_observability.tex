...
\chg{정적 RTO의 한계를 극복하려면 복구 환경의 상태 변화를 실시간으로 파악할 수 있어야 한다. 이러한 상태 파악 능력을 이론적으로 뒷받침하는 개념이 관측성(observability)이며, 2.3.1(제어 이론에서의 관측성)에서는 제어 이론에서 출발한 관측성 개념과 그 현대적 확장을 검토한다.}

%% -------------------------------------------------------------------
\subsection{제어 이론에서의 \m{관측성(Kalman, 1960)}}
\label{subsec:kalman-observability}
%% -------------------------------------------------------------------

관측성(observability)의 개념은 \citet{kalman1960control}이 제어 이론에서 체계적으로 형식화하였다.
Kalman은 \m{식~(\ref{eq:lti-system})과 같은} 선형 시불변(Linear Time-Invariant, LTI) 시스템
\begin{equation}
  \dot{x}(t) = Ax(t) + Bu(t), \quad y(t) = Cx(t)
  \label{eq:lti-system}
\end{equation}
\m{을 대상으로, 출력} $y(t)$\m{를 관측만으로} 초기 상태 $x(0)$를 유일하게 결정할 수 있는지를
판별하는 기준으로 관측성 행렬(observability matrix)의 계수(rank) 조건을 제시하였다.
관측성 행렬\m{은 식~(\ref{eq:obs-matrix})와 같이}
\begin{equation}
  \mathcal{O} = \begin{bmatrix} C \\ CA \\ CA^2 \\ \vdots \\ CA^{n-1} \end{bmatrix}
  \label{eq:obs-matrix}
\end{equation}
\m{정의되며, 이 행렬이} 완전 계수(full rank)일 때 시스템이 완전 관측 가능(completely observable)하다.
...
\chg{Kalman 이후 제어 이론에서 관측성은 상태 추정(state estimation)의 토대로 발전하였다. \citet{luenberger1966observers}는 관측성 조건이 충족되는 선형 시스템에서 직접 측정되지 않는 내부 상태를, 가용한 출력과 입력만으로 구동되는 보조 동역학계인 상태 관측기(state observer)를 통해 재구성할 수 있음을 보였다. 이로써 관측성은 가부 판별을 넘어 관측 가능한 시스템에서는 내부 상태를 실제로 복원할 수 있다는 구성적 의미를 \m{지니게 되었고}, 이후 상태 추정과 피드백 제어의 핵심 원리로 자리 잡았다. \m{본 연구는} 관측성을 복구 환경의 내부 상태에 대한 가시성으로 \m{해석하는데, 이는} 출력으로부터 내부 상태를 복원하는 역량이라는 제어 이론적 함의를 BCM 영역으로 확장한 것이다.}
...
%% --- BCM 맥락의 정규화 ---
\subsubsection{\m{〔삭제 대상〕} BCM 맥락에서의 관측성 정규화}
\label{subsubsec:obs-normalization}

\m{본 연구는} Kalman의 제어 이론적 관측성 개념을 BCM 영역으로 확장하여 적용한다.
\chg{Kalman의 원래 정식화는 관측성 행렬의 계수 조건을 통해 시스템이 ``완전 관측
가능한가''를 가능과 불가능의 이분법으로 판별하고 있다. 그러나 \chg{BCM과 재해복구} 실무에서 조직의 관측성은 이분법으로 주어지지 않는다.
...
정도가 연속적으로 변한다.} 그 결과 조직의 관측성은 0과 1 \m{사이에} 연속적으로 분포한다. \m{이에 본 연구는} Kalman의
...
단축량의 정량적 모형화가 \m{가능하기} 때문이다.}
이때 $O_{\text{raw}} = 0$은 시스템 상태를 전혀 파악할 수 없는 상태를,
$O_{\text{raw}} = 1$은 완전히 관측 가능한 상태를 의미한다.


%% --- 관측성-복구 능력 구조적 연결 ---
\subsubsection{\m{〔삭제 대상〕} 관측성과 복구 능력의 구조적 연결}
\label{subsubsec:obs-recovery-link}

\m{관측성과 복구 능력 사이에는 구조적 연결이 존재한다~\citep{avizienis2004basic, hollnagel2006resilience}.}
시스템 상태를 보다 정확히 파악할 수 있을수록 장애의 근본 원인(root cause)을
신속히 진단할 수 있고, 이에 따라 복구에 소요되는 시간이 단축될 수 있다.
\citet{avizienis2004basic}는 가용성(availability)·신뢰성(reliability)·안전성(safety)·무결성(integrity)을 포괄하는 의존성(dependability)의 분류 체계에서
장애 진단 능력이 시스템의 가용성에 \m{직접} 영향을 미침을 논의하였다.
\citet{hollnagel2006resilience}는 복원공학(Resilience Engineering) 관점에서 시스템의
\m{예측·감시·대응·학습} 능력을 \m{강조하였는데}, 이 중 감시(monitoring) 능력이 본질적으로
관측성에 \m{해당한다}.
\chg{본 연구는 이 두 이론을 관측성과 복구 시간 사이의 단조 감소 관계로 \m{반영한다}.
\m{곧} \citet{avizienis2004basic}에서 장애 진단 능력이 가용성에 기여한다는 인과를,
\citet{hollnagel2006resilience}에서는 감시 능력이 복원력의 핵심이라는 명제를 각각
가져왔다. 본 모델은 이 ``관측성에서 진단으로, 진단에서 복구 시간 단축으로''\m{ 이어지는} 인과를
관측성 채널의 하향 조정량 $E_{O,\text{bnd}} \leq 0$으로 \m{구현하여,} 관측성이 높을수록
$\DRTO$를 기준 RTO $R_0$ 아래로 끌어내리는 음의 효과로 \m{정량화한다}. \m{이렇게} 적용한 이유는 이론이 제시한 인과 관계를 \m{모델의}
단조 감소 관계로 직접 \m{반영하여,} 관측성 투자에 따른 복구 시간 단축을 정량적으로
\m{예측하기} 위함이다.}


%% --- SRE와 세 가지 핵심 요소 ---
\subsubsection{\m{〔삭제 대상〕} \chg{현대 IT 운영의 관측성 SRE와 세 가지 핵심 요소(three pillars)}}
...
\m{메트릭(metrics)·로그(logs)·트레이스(traces)}를} 체계화하였다.
\m{이 세 요소를 통합적으로 활용함으로써} 조직은 예상치 못한 장애 상황에서도
시스템의 내부 상태를 효과적으로 파악할 수 있다.
\p{[}그림~\ref{fig:obs-three-pillars}\p{]}는 이러한 세 가지 핵심 \m{요소}의 구성 관계를 도식화한 것이다.


%% --- 운영화 매핑 ---
\subsubsection{\m{〔삭제 대상〕} \chg{데이터 유형에서} \chg{도구와 목적} \chg{차원으로의 운영화 매핑}}
\label{subsubsec:obs-operationalization}

이러한 세 가지 핵심 \m{요소} 분류는 관측성을 구성하는 \textbf{데이터 유형(data primitives)} 차원의
...
APM과 SIEM은 각각 \chg{\m{메트릭·로그·트레이스}}를 통합 활용하되 측정 목적과 주된 데이터 출처가
상이하므로 분리 산출 후 가중 합성($O_{\text{raw}} = \alpha \cdot O_{\text{APM}}
+ (1-\alpha) \cdot O_{\text{SIEM}}$)으로 \m{통합한다.} 이론적 분류와 운영화 모델의 매핑 관계는
5.3.4(실무적 기여)의 조작화 예시에서 참고용으로 다룬다.
...
\toprule
\textbf{비교 기준} & \textbf{\m{모니터링(Monitoring)}} & \textbf{\m{관측성(Observability)}} \\
\midrule
정의 &
...
\addlinespace
범위 &
  사전 정의된 이상 징후 \m{탐지(known-unknowns)} &
  예상하지 못한 장애의 근본 원인 추적 \m{포함(unknown-unknowns)} \\
\addlinespace
BCM 역할 &
...
\chg{이처럼 관측성은 복구 환경의 가시성을 좌우하여 복구 시간에 직접 영향을 미치지만, 복구 시간에 내재한 변동성 자체를 설명하지는 못한다. 따라서 이 변동성을 정량화하는 불확실성 모델링 검토가 요구되고 있다.}

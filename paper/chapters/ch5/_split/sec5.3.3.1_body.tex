%% --- 위기시: 에스컬레이션 대응 체계 ---
\subsubsection{\syncr{위기시 에스컬레이션 대응 체계}}
...
\midrule
1 & $\eta < 0.8$ & 안정 (Green) & 복구 팀 자체 & 정상 복구 절차 수행; 관측성에 의한 $\DRTO$ 단축 효과 활성 \\
\addlinespace
2 & $0.8 \leq \eta < 1.0$ & 주의 (Yellow) & 팀장 보고 & 불확실성 증가 모니터링; $\DRTO$ 갱신 주기 단축 (15분); 추가 자원 대기 \\
\addlinespace
3 & $1.0 \leq \eta < 1.5$ & 경고 (Orange) & 부서장 보고 & $\DRTO > R_0$로 기준 RTO 초과; 추가 자원 투입; 외부 지원 요청; 이해관계자 긴급 통보 \\
\addlinespace
4 & $\eta \geq 1.5$ & 위기 (Red) & 경영진 보고 & 위기관리 모드 전환; 사업 연속성 비상 계획(BCP) 전면 가동; CVaR$_{99}$ 기준 최악 시나리오 대비 \\
\bottomrule
\end{tabularx}
\end{table}
...
\caption{$\eta$ 기반 에스컬레이션 임계값 다이어그램 (Green / Yellow / Orange / Red)}
...
$\eta = 1.5$은 $\DRTO = 1.5 \times R_0 = 9.0$\,h에 대응하며,
...
예컨대, $\eta = 1.3$(Orange 등급)이 Core 영역(C2)에서 관측되면

...
\chg{표의 공통 파라미터는 다음과 같다. 곡률 매개변수는 $\kappa = 1.2$로 \jrev{\citet{lee2026drto}}에서 확정된
...
불확실성 분포이며, $R_0 / R_{\max} = 1:4$ 비율을 유지한다(연구모형 기본 설정). $O_{\text{raw}}$ 범위는 해당 산업의
전형적 모니터링 성숙도를 반영한다.}
...
산업별 모델 선택의 핵심 기준은 \textbf{파레토 위험의 존재 여부}이다.
...
위험은 낮다. 예컨대 정상적 IT 장애 복구에 대해 랜섬웨어 대응 수준의 인력과 예비
...
모델 선택은 단순한 분포 가정의 문제가 아니라 자원 배정의 비용과 위험을 균형 짓는

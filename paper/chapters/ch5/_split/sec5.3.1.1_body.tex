...
이에 기반하여 BCMS 성숙도를 5단계로 구분하는 모형을
제안한다(\p{[}표~\vref{tab:maturity-5level}\p{]}).

각 임계값은 본 연구의 시뮬레이션 결과에 기반한다\chg{.}
$\eta = 0.6$은 C1 조건의 하위 25\% 수준으로 관측성이 극대화된 상태,
...
3 & 정의 & $0.8 \leq \eta < 1.0$ & \textit{Defined} &
  공식 BCM 프로세스 수립. 관측성과 불확실성 상쇄 ($\DRTO \approx R_0$) \\
\addlinespace
...
각 성숙도 단계별 구체적 전환 전략, 위기시 정책 활용 방안, 도입 로드맵은

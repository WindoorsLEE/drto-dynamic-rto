...
\syncr{\subsubsection{본 절의 성격과 한계}}

\syncr{\textbf{본 절은 $\DRTO$ 프레임워크의 실무 적용 방향성을 보여주기 위한
...
공식적 기여이나, 본 절의 조작화와 시나리오는 \textbf{향후 연구를 위한
...
\syncr{본 절은 측정이론의 고전적 원칙을 따라, 해당 하위 지표가 현장의
\textbf{기존 운영 시스템}(APM 도구, SIEM 도구, 장애 이력 데이터베이스,
CVE 공개 데이터베이스, 자산 대장 등)\textbf{에서 직접 \chg{관측되고 기록되는} 원시값}에
...
\begin{table}[htbp]
\centering
\caption{\syncr{\chg{관측성과 불확실성, $k_O$}의 1차 조작화 예시 (참고용)}}
\label{tab:app-operationalization}
\small
\syncr{\begin{tabularx}{\textwidth}{l l X l}
\toprule
\textbf{주 변수} & \textbf{1차 차원} & \textbf{원시 지표 (정지점)} & \textbf{측정 출처} \\
\midrule
...
& & $T_{\text{mttr}}$: 평균 탐지시간(정규화) & SIEM 도구 \\
\midrule
...
범위로 본 연구 모델의 $U_{\text{raw}}$ 정의와 일치)}한다.}
...
\syncr{관측성 가중치 $k_O \in [0,1]$은 조직이 관측성에 얼마나 투자했는지를
...
\syncr{``(주)한빛정밀''(가상기업명, 동명의 실제 기업과 무관)은
...
$E_{\text{high}} = 0.08$, $T_{\text{mttr}} = 0.65$, 커버리지율 $= 0.72$.}

\syncr{합성 결과는 다음과 같다\chg{.} $O_{\text{APM}} = 0.933$, $O_{\text{SIEM}} = 0.785$,
$O_{\text{raw}} = 0.889$, $k_O = 0.72$, $U = 0.55$로 산출되어
$\eta \approx 1.05$ (Level 2 Repeatable) 판정. 모니터링 커버리지 확대 및
미대응 이벤트 축소가 개선 필요 영역으로 도출.}

\syncr{\chg{2단계인 평상시 개선 활동(T-365일부터 T-1일)에서는} 분기별 복구 훈련 도입, 모니터링 자산 확대(커버리지율 0.72 → 0.88),
SIEM 튜닝($E_{\text{high}}$ 0.08 → 0.03), 장애 이력 DB 정비 수행.
금년 재평가\chg{에서} $\eta \approx 0.82$\chg{를 달성하여} \textbf{Level 4 Managed}로 \chg{향상되었다}.
월별 $\eta$ 추이를 경영진 대시보드에 정착.}

\syncr{\chg{3단계인 위기 발생 및 에스컬레이션(T+0h--T+6h)의} 가상 시나리오는 생산라인 제어 PLC 장애와 보조 전원계통 이상
(인명 피해 없음, 설비 손상만 발생). 시간대별 $\eta$ 추이와 등급 변화는
\tabref{tab:app-lifecycle-eta}에 정리.}

\begin{table}[htbp]
\centering
\caption{\syncr{가상 기업 4단계 라이프사이클 $\eta$ 추이 (참고 시나리오)}}
...
\midrule
$T{-}365$d & 1단계 진단 & 초기 상태 (Level 2) & 1.05 & Yellow \\
$T{-}1$d & 2단계 개선 후 & 모니터링 확대 + SIEM 튜닝 (Level 4) & 0.82 & Green \\
\midrule
$T{+}0$h & 3단계 정상 & 사건 직전 & 0.82 & Green \\
...
$T{+}6$h & 3단계 부분복구 & 주요 지표 안정화 & 1.08 & Yellow \\
\midrule
$T{+}72$h & 4단계 환류 & 사후 분석 후 재평가 & 0.84 & Green \\
\bottomrule
\end{tabularx}}
\end{table}
...
연동 검토\chg{로 단계화된다}(\subsubsecref{app:crisis-policy} 참조).}

\syncr{\chg{4단계인 복구와 환류(T+24h--T+72h)의} 사후 분석에서는 실제 복구 시간 $T_{\text{actual}} = 8.2$h vs $\DRTO$ 예측 7.6h
→ 상대 오차 $E_r = 7.3\%$ (수용 범위). $O(t)$ 사각지대로 전원계통 센서
커버리지 부족을 식별하고, 성숙도 ``Managed → Optimized'' 이행 로드맵에
반영. 이번 사건을 $U_{\text{hist}}$ DB에 환류하여 익년 진단의 입력으로 사용.}
...
\begin{table}[htbp]
\centering
\caption{\syncr{원시 지표별 측정 도구 매핑 예시 (참고용, 도구 권장 아님)}}
\label{tab:app-tool-mapping}
\small
\syncr{\begin{tabularx}{\textwidth}{l l X}
\toprule
\textbf{원시 지표} & \textbf{측정 도구 유형} & \textbf{대표 도구 (참고)} \\
\midrule
...
\syncr{본 절의 조작화와 시나리오는 참고용 예시이며, 다음 네 영역에서
후속 실증 연구가 필요하다.}

\syncr{\chg{첫째, 가중치 실증 튜닝이다.} $w_i$, $v_i$, $\alpha$, $\beta$를 \chg{산업별과
조직 규모별}로 조정하는 다중 회귀 또는 최적화 연구가 필요하며,
...
\syncr{종합적으로 본 절은 $\DRTO$ 프레임워크의 실무 적용 \textbf{방향성}을

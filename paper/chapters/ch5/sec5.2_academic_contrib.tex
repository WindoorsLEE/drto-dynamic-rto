...
\begin{p5add}
본 절은 본 연구가 BCM\chg{과 재해복구}(DR) 학문 분야의
지식 체계에 새롭게 기여한 바를 \chg{i) 이론적, ii) 방법론적, 그리고 iii) 실증적} 세 차원에서
종합한다. 이론적 기여(5.2.1항)에서는 $\DRTO$ 모델의 수학적 구조와
이중채널 감쇠 메커니즘의 신규성을, 방법론적 기여(5.2.2항)에서는 비중첩
...
\begingroup
  ...
  \input{chapters/ch5/_split/sec5.2.1_body}

  % --- 보완: 5.2 이론적 시사점 (기술적 세부 설명 보완) ---
  \input{chapters/ch5/sec5.2.1_theoretical}

  % --- 5.8.1 (이론적 기여) — 5.1.2와 3중 주제 중복으로 5-2 삭제 ---
  \begin{p5del}{현 5.8.1 이론적 기여 — 5.1.2의 1)/2)/3)과 동일 주제 중복}
  \input{chapters/ch5/_split/sec5.2.1_contributions}
  \end{p5del}
\endgroup
...
\begingroup
  ...
  \input{chapters/ch5/sec5.2.2_methodological}
  \input{chapters/ch5/_split/sec5.2.2_contributions}
\endgroup
...
\begin{p5add}
본 항은 본 연구가 시뮬레이션과 전문가 평가의 결합 검증을 통해 달성한
실증적 기여를 정리한다.

첫째, \textbf{\jrev{10개 연구가설}(H1--\jrev{H10})이 사전 설정 기준을 모두 충족하여 모두
채택}되었다. 시뮬레이션 검증(H1--\jrev{H3}), 회귀 검증(\jrev{H4}--\jrev{H5}), 강건성 검증(\jrev{H6}--\jrev{H7}),
이중채널 검증(\jrev{H8}), 전문가 평가(\jrev{H9}--\jrev{H10})의 6단계 다층 검증 프레임워크에서
...
45개 회귀계수 중 기각이 0개로, 시뮬레이션 결과가 특정 난수열에 의존하지 않는
강건성을 정량적으로 입증하였다.

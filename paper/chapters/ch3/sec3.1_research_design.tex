...
\chg{프레임워크는 결정론적 기반 검증에서 출발하여 통계적 검증과 전문가 평가로 단계적으로 심화한다. 먼저 기반 검증(L0, Foundation Verification)은 Golden Compare로서, 7개 산업 시나리오의 대표 매개변수를 $\DRTO$ 수식에 직접 대입하여 해석적 기댓값과 코드 출력을 비교하는 결정론적 검증이다. 순서 제약, 물리적 경계, $z_O$ 동일성, P99 비율 일관성의 네 범주에 걸친 총 47개 검증 포인트로 구성되며, 경계 보장(H2)과 순서 보존(\jrev{H3})을 결정론적으로 사전 검증한다. 47개 포인트 전체에서 PASS를 달성해야만 이후 L1에서 L5까지의 가설 검증이 의존하는 수식 구현이 단일 기준 정의(SSOT)와 정합함이 보장된다. 이 결과는 \subsecref{subsec:golden_compare}를 참조하고, 7개 시나리오의 전체 계산과 47개 점별 결과는 공개 재현 패키지(GitHub)에 수록한다.}

\chg{이 L0 기반 위에서 시뮬레이션 검증(L1)은 $n = 10{,}000$ 몬테카를로 시뮬레이션으로 관측성 효과(H1), 경계 보장(H2), 조건 순서(\jrev{H3})를 확인하여 모델의 기본 행동을 검증한다. 이때 H2와 \jrev{H3}는 L0에서 결정론적으로 사전 확인된 뒤 본 단계의 확률적 표본으로 재검증되며, 곡률 매개변수 $\kappa = 1.2$의 최적성은 \citet{lee2026drto}에서 확정된 전제로서 본 연구의 가설 검증 대상이 아니다. 다음 회귀 검증(L2)은 2차 다항 회귀 모델의 설명력(\jrev{H4})과 교호작용 유의성(\jrev{H5})을 통해 응답 곡면의 구조적 특성을 규명하고, 강건성 검증(L3)은 시드 독립성(\jrev{H6})과 다중 시드 강건성(\jrev{H7})을 통해 결과의 재현성과 대표성을 확인하는데, 표본 $n = 10{,}000$과 시드 $10{,}000$개를 곱한 총 $10^8$회 계산으로 통계적 안정성을 보장한다. 다음으로 이중채널 검증(L4)은 P85.8 기준 분할 회귀로 이중채널 비선형 감쇠 효과의 존재와 크기를 검증하며, L1에서 L3까지의 결과를 종합하는 핵심 발견으로서 실무적 정책 시사점의 근거를 제공한다. 마지막으로 전문가 평가(L5)는 BCM 및 재해복구 전문가 5명의 정량적 설문(\jrev{H9})과 정성적 인터뷰(\jrev{H10})를 통해 모델의 이론적 타당성과 실무 적용 가능성을 삼각검증한다.}
...
\midrule
입력 & 고정 \m{매개변수(시나리오)} & 확률 \m{분포($n = 10{,}000$ 표본)} \\
출력 비교 & 해석적 기댓값 = 코드 \m{출력(정확 일치)} & 분포·평균·검정 통계량 \\
결론 형식 & PASS / FAIL (이진) & 가설 채택/기각, \m{효과 크기} \\
변동성 & \m{없음(재실행 시 동일)} & 시드·표본에 따른 변동 \\
검증 대상 & 수식 구현 \m{정합성(코드와 이론)} & 모델 거동의 통계적 성질 \\
...
%% -------------------------------------------------------------------
\chg{이상의 검증 프레임워크를 실제로 수행하기 위한 시뮬레이션의 구체적 구성, 곧 표본 크기와 분포 매개변수, 난수 생성 방법, 변수별 시드 배정 규칙과 그 독립성 검증은 3.2(시뮬레이션 설계)에서 통합적으로 기술한다.}

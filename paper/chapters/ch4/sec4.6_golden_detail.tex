...
\begin{align*}
  ...
  E_{O,bnd} &= -6.0 \times 0.1489 = \erf{-0.894}{-0.893}\;\text{h} \\
  \DRTO_{C1} &= 6.0 + (\erf{-0.894}{-0.893}) + 0 = \erf{5.106}{5.107}\;\text{h}, \quad \eta_{C1} = 0.851
\end{align*}
...
\begin{align*}
  ...
  \DRTO_{C2} &= 6.0 + (\erf{-0.894}{-0.893}) + 5.244 = 10.350\;\text{h}, \quad \eta_{C2} = 1.725
\end{align*}
...
\begin{align*}
  ...
  S(z_U) &= \frac{2}{1+e^{-0.600}} - 1 = 0.2913 \\
  E_{U,bnd} &= (24.0 - 6.0) \times 0.2913 = 5.244\;\text{h} \\
  \DRTO_{C3} &= 6.0 + (\erf{-0.894}{-0.893}) + 5.244 = 10.350\;\text{h}, \quad \eta_{C3} = 1.725
\end{align*}
...
\chg{검증 결과} $0 < \text{C1}(\erf{5.106}{5.107}) < \text{C2}(10.350) < \text{C3}_{\text{P99}}(\erf{22.881}{22.657}) < 24.0$\chg{으로 물리적 경계를 충족한다.}
...
2 & 제조업    & \erf{3.997}{3.218} & \erf{5.670}{8.567}  & \erf{8.872}{14.757}  & PASS \\
3 & 금융      & \erf{0.999}{0.795} & \erf{1.398}{2.376}  & \erf{2.218}{3.746}  & PASS \\
4 & 의료      & \erf{1.998}{1.598} & \erf{2.830}{4.522}  & \erf{4.436}{7.448}  & PASS \\
5 & IT서비스   & \erf{0.500}{0.368} & \erf{0.704}{1.036}  & \erf{1.109}{1.810}  & PASS \\
6 & 물류      & \erf{5.998}{5.091} & \erf{8.443}{14.153}  & \erf{13.224}{22.713} & PASS \\
7 & 소매      & \erf{2.998}{2.472} & \erf{4.221}{6.711}  & \erf{6.612}{11.205}  & PASS \\
\midrule
...
2 & 제조업    & 16.0 & 4.000 & \erf{3.221}{3.218} & \erf{8.872}{14.757}  & PASS (4건) \\
3 & 금융      & 4.0  & 1.000 & \erf{0.796}{0.795} & \erf{2.218}{3.746}  & PASS (4건) \\
4 & 의료      & 8.0  & 2.000 & \erf{1.599}{1.598} & \erf{4.436}{7.448}  & PASS (4건) \\
5 & IT서비스   & 2.0  & 0.500 & \erf{0.369}{0.368} & \erf{1.109}{1.810}  & PASS (4건) \\
6 & 물류      & 24.0 & 6.000 & \erf{5.095}{5.091} & \erf{13.224}{22.713} & PASS (4건) \\
7 & 소매      & 12.0 & 3.000 & \erf{2.474}{2.472} & \erf{6.612}{11.205}  & PASS (4건) \\
\midrule
...
시나리오에 무관하게 안정적인 \erf{값($\approx 1.57$)}{범위(평균 $\approx 1.66$)}을 유지하는지 검증한다.
원시 입력 $U_{\text{raw}}$의 P99 비율은 파레토/가우시안 $\approx \erf{3.95}{4.1}$이나,
시그모이드 바운딩을 거치면서 비선형 압축이 적용되어
$\DRTO$ 수준에서의 비율은 \erf{$\approx 1.57$로 수렴한다}{$\approx 1.6$--$1.8$ 수준으로 압축된다}(\p{[}표~\ref{tab:app-golden-ratio}\p{]}).
...
\textbf{\#} & \textbf{시나리오}
  ...
  & \textbf{비율} & \textbf{판정} \\
\midrule
2 & 제조업    & \erf{5.670}{8.567}  & \erf{8.872}{14.757}  & \erf{1.56}{1.72} & PASS \\
3 & 금융      & \erf{1.398}{2.376}  & \erf{2.218}{3.746}  & \erf{1.59}{1.58} & PASS \\
4 & 의료      & \erf{2.830}{4.522}  & \erf{4.436}{7.448}  & \erf{1.57}{1.65} & PASS \\
5 & IT서비스   & \erf{0.704}{1.036}  & \erf{1.109}{1.810}  & \erf{1.58}{1.75} & PASS \\
6 & 물류      & \erf{8.443}{14.153}  & \erf{13.224}{22.713} & \erf{1.57}{1.60} & PASS \\
7 & 소매      & \erf{4.221}{6.711}  & \erf{6.612}{11.205} & \erf{1.57}{1.67} & PASS \\
\midrule
\multicolumn{4}{l}{\textbf{합계: 6/6 PASS}} & \textbf{평균 \erf{1.57}{1.66}} & \textbf{100\%} \\
\bottomrule
\end{tabular}
\end{table}

\chg{P99 비율의 평균은 \erf{$\approx 1.57$}{$\approx 1.66$}로 시나리오 간 \erf{높은 }{}일관성을 보이며, 이 비율은 파레토 분포의 P99와
...
P99 비율 일관성 & $\DRTO_{C3,\text{P99}} / \DRTO_{C2,\text{P99}} \approx \erf{1.57}{1.66}$           & 6  & 100\% PASS \\
\midrule

...
파레토의 이론적 P99($U_{\text{raw}} = 21.85$)를 대입하면
...
2 & 제조업    & 4.000 & 3.218 & 0.805 & 19.5\% &  \erf{6.382}{6.044} & \erf{1.595}{1.511} & \erf{14.757}{14.512} & \erf{3.689}{3.628} \\
3 & 금융      & 1.000 & 0.795 & 0.795 & 20.5\% &  \erf{1.751}{1.206} & \erf{1.751}{1.206} &  \erf{3.746}{3.086} & \erf{3.746}{3.086} \\
4 & 의료      & 2.000 & 1.598 & 0.799 & 20.1\% &  \erf{3.345}{2.736} & \erf{1.673}{1.368} &  \erf{7.448}{6.908} & \erf{3.724}{3.454} \\
5 & IT서비스   & 0.500 & 0.368 & 0.736 & 26.4\% &  \erf{0.763}{0.529} & \erf{1.527}{1.059} &  \erf{1.810}{1.353} & \erf{3.620}{2.705} \\
6 & 물류      & 6.000 & 5.091 & 0.849 & 15.1\% & \erf{10.532}{10.037} & \erf{1.755}{1.673} & \erf{22.713}{22.509} & \erf{3.786}{3.751} \\
7 & 소매      & 3.000 & 2.472 & 0.824 & 17.6\% &  \erf{4.995}{4.744} & \erf{1.665}{1.581} & \erf{11.205}{11.062} & \erf{3.735}{3.687} \\
\bottomrule
\end{tabular}
\end{table}
...
두 조건이 일치한다. $\DRTO_{C3,\text{P99}}$는 파레토 이론적 P99($U_{\text{raw}} = 21.85$) 대입값으로, 꼬리 영역에서
...
\toprule
\textbf{\#} & \textbf{시나리오}
  & \textbf{$\DRTO_{C1\erf{,\text{P99}}{}}$ (h)} & \textbf{$\DRTO_{C2,\text{P99}}$ (h)}
  & \textbf{$\DRTO_{C3,\text{P99}}$ (h)} & \textbf{판정} \\
\midrule
1 & 기준선    & \erf{5.998}{5.107} & \erf{8.443}{13.879}  & \erf{13.224}{22.657} & PASS \\
2 & 제조업    & \erf{3.997}{3.218} & \erf{5.670}{8.043}  & \erf{8.872}{14.512}  & PASS \\
3 & 금융      & \erf{0.999}{0.795} & \erf{1.398}{1.516}  & \erf{2.218}{3.086}  & PASS \\
4 & 의료      & \erf{1.998}{1.598} & \erf{2.830}{3.567}  & \erf{4.436}{6.908}  & PASS \\
5 & IT서비스   & \erf{0.500}{0.368} & \erf{0.704}{0.652}  & \erf{1.109}{1.353}  & PASS \\
6 & 물류      & \erf{5.998}{5.091} & \erf{8.443}{13.418}  & \erf{13.224}{22.509} & PASS \\
7 & 소매      & \erf{2.998}{2.472} & \erf{4.221}{6.328}  & \erf{6.612}{11.062}  & PASS \\
\midrule
...
\midrule
1 & 기준선    & 24.0 & 6.000 & \erf{5.106}{5.107} & \erf{13.224}{22.657} & PASS (4건) \\
2 & 제조업    & 16.0 & 4.000 & \erf{3.221}{3.218} & \erf{8.872}{14.512}  & PASS (4건) \\
3 & 금융      & 4.0  & 1.000 & \erf{0.796}{0.795} & \erf{2.218}{3.086}  & PASS (4건) \\
4 & 의료      & 8.0  & 2.000 & \erf{1.599}{1.598} & \erf{4.436}{6.908}  & PASS (4건) \\
5 & IT서비스   & 2.0  & 0.500 & \erf{0.369}{0.368} & \erf{1.109}{1.353}  & PASS (4건) \\
6 & 물류      & 24.0 & 6.000 & \erf{5.095}{5.091} & \erf{13.224}{22.509} & PASS (4건) \\
7 & 소매      & 12.0 & 3.000 & \erf{2.474}{2.472} & \erf{6.612}{11.062}  & PASS (4건) \\
\midrule
...
6 & 물류      & \erf{0.306}{0.305} & \erf{0.306}{0.305} & \erf{0.306}{0.305} & PASS \\
...
시나리오에 무관하게 안정적인 \erf{값($\approx 1.57$)}{범위(평균 $\approx 1.88$)}을 유지하는지 검증한다.
원시 입력 $U_{\text{raw}}$의 P99 비율은 파레토/가우시안 $\approx 3.95$이나,
시그모이드 바운딩을 거치면서 비선형 압축이 적용되어
$\DRTO$ 수준에서의 비율은 \erf{$\approx 1.57$로 수렴한다}{$\approx 1.7$--$2.1$ 수준으로 압축된다}(\p{[}표~\ref{tab:app-golden-ratio}\p{]}).
...
\textbf{\#} & \textbf{시나리오}
  ...
  & \textbf{비율} & \textbf{판정} \\
\midrule
2 & 제조업    & \erf{5.670}{8.043}  & \erf{8.872}{14.512}  & \erf{1.56}{1.80} & PASS \\
3 & 금융      & \erf{1.398}{1.516}  & \erf{2.218}{3.086}  & \erf{1.59}{2.04} & PASS \\
4 & 의료      & \erf{2.830}{3.567}  & \erf{4.436}{6.908}  & \erf{1.57}{1.94} & PASS \\
5 & IT서비스   & \erf{0.704}{0.652}  & \erf{1.109}{1.353}  & \erf{1.58}{2.07} & PASS \\
6 & 물류      & \erf{8.443}{13.418}  & \erf{13.224}{22.509} & \erf{1.57}{1.68} & PASS \\
7 & 소매      & \erf{4.221}{6.328}  & \erf{6.612}{11.062} & \erf{1.57}{1.75} & PASS \\
\midrule
\multicolumn{4}{l}{\textbf{합계: 6/6 PASS}} & \textbf{평균 \erf{1.57}{1.88}} & \textbf{100\%} \\
\bottomrule
\end{tabular}
\end{table}

\chg{P99 비율의 평균은 \erf{$\approx 1.57$}{$\approx 1.88$}로 시나리오 간 \erf{높은 }{}일관성을 보이며, 이 비율은 파레토 분포의 P99와
...
P99 비율 일관성 & $\DRTO_{C3,\text{P99}} / \DRTO_{C2,\text{P99}} \approx \erf{1.57}{1.88}$           & 6  & 100\% PASS \\
\midrule

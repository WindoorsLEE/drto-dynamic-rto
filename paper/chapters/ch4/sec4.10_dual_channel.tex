...
Tail($>$ P85.8, $n = 1{,}420$)로 분할하여 회귀분석을 수행하면,
...
\chg{반면 Tail 채널은 비선형 구간에서 포화 효과를 겪는다.}
...
요컨대, 파레토 불확실성은 시그모이드의 비선형 구간을 활성화하여
...
\jrev{4.4절(전진 선택법)}의 분석에서 C2와 C3의 변수 진입 순서가 질적으로
다르다는 발견은 교차분석 관점에서 중요한 의미를 갖는다.
...
비선형 교호작용에 의해 설명됨을 시사하며,
2차교호작용 모형의 필요성을 교차분석 관점에서 뒷받침한다.
...
\jrev{\figref{fig:ch5_dual_channel_mechanism}\refpo{fig:ch5_dual_channel_mechanism}{은}{는}} 이중채널 감쇠의
메커니즘을 도식으로 나타낸 것이다.
...
%% -------------------------------------------------------------------
\subsection{\vrev{이중채널 감쇠의 실무적 함의}}
...
\chg{끝으로 H8의 검증 결론을 정리하면,} \mrev{H8은 ``Tail ($>$P85.8) 영역에서 $k_O$ 계수의 절댓값이 Core 영역의
$\leq 1.0$배''를
기준으로 한다. Tail $|\beta_{k_O}|
= 0.415$ vs.\ Core $|\beta_{k_O}| = 0.798$의 비율이 $0.415/0.798 = 0.52
< 1$이므로, \textbf{H8은 채택된다}. 이 감쇠 효과는 분포 환경에 의존하는
관측성 효과의 비선형성을 정량적으로 입증한 본 논문의 \emph{독자적
발견}이다.}

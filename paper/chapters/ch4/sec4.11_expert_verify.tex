...
\textbf{혼합연구방법(Mixed Methods Research)}으로 설계되었으며,
...
본 연구의 전문가 검증은 사회과학적 양적 연구에서 잠재변수 간
인과관계를 규명하는 구조방정식 모델링(SEM)이나 \chg{탐색적, 확인적}
요인분석과 목적이 근본적으로 다르다.
...
따라서 전문가 검증의 목적은 잠재변수 간 관계 탐색이 아니라,
...
\input{chapters/ch4/sec4.11_profile}
...
\chg{\p{[}그림~\vref{fig:expert-verification-process}\p{]}는 전문가 패널 검증의 전체 절차를 안내 메일 발송부터 결과 종합까지 6단계로 체계화한 것이다. 설문은 모델 타당성(B), 파라미터 적정성(C), 실무 적용성(D)에 더하여 산업 적용과 $\eta$ 활용(E)을 검증하며, 약 30분의 심층 인터뷰로 정량적 설문 결과를 보충하는 혼합연구방법(Mixed Methods) 삼각검증 설계를 따른다.}
...
설문 도구의 내용 타당도는
\textbf{내용타당도지수(Content Validity Index, CVI)}%
~\citep{lynn1986determination}\pdel{와
\textbf{내용타당도비율(Content Validity Ratio, CVR)}%
~\citep{lawshe1975quantitative}}로 평가하였다.
\padd{내용타당도비율(CVR)~\citep{lawshe1975quantitative}은 Lawshe(1975) 원표가 패널 5명에서
...
설문에 대해 Cronbach's $\alpha$로 평가하였다
(\jrev{\tabref{tab:ch5_alpha}}).
...
\begin{table}[htbp]
  \centering
  \caption{\jrev{내적 일관성 신뢰도 분석 (Cronbach's $\alpha$)}}
  \label{tab:ch5_alpha}
  \small
  \begin{tabular}{lccl}
    \toprule
    ...
    \midrule
    모델 타당성 (Section B)             & 6  & 0.58 & $\triangle$ 미달$^{**}$ \\
    파라미터 적정성 (Section C)         & 6  & 0.51 & $\triangle$ 미달$^{**}$ \\
    실무 적용성 (Section D)             & 6  & 0.70 & 경계 충족 \\
    산업적용·$\eta$ 활용 (Section E)     & 9  & 0.80 & \cmark 양호 \\
    \midrule
...
\chg{해석 기준은 $\geq 0.90$ 우수, $\geq 0.80$ 양호, $\geq 0.70$ 수용 가능이며~\citep{nunnally1994psychometric},
...
Krippendorff's $\alpha$로 평가하였다 (\jrev{\tabref{tab:ch5_icc}}).

\begin{table}[htbp]
  \centering
  \caption{\jrev{평가자 간 일치도 분석 ($n = 5$ 평가자, $k = 27$ 문항)}}
  \label{tab:ch5_icc}
  \small
  \begin{tabular}{llcl}
    \toprule
    측정치 & 값 & 기준 & 해석 \\
    \midrule
    S-CVI/Ave & 0.985 & $\geq 0.90$ & \cmark 우수 (주된 타당도 지표) \\
...
4대 기준에 따라 확보하였다(\jrev{\tabref{tab:ch5_trustworthiness}}).
...
\input{chapters/ch4/sec4.11_reliability_formulas}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccccl}
    ...
    \midrule
    모델 타당성 (B, 6문항)         & 4.57 & 0.30 & 4.19  & 0.007 & 1.87 \\
    파라미터 적정성 (C, 6문항)     & 4.40 & 0.28 & 3.21  & 0.016 & 1.43 \\
    실무 적용성 (D, 6문항)         & 4.57 & 0.35 & 3.67  & 0.011 & 1.64 \\
    산업적용·$\eta$ 활용 (E, 9문항) & 4.36 & 0.35 & 2.30  & 0.042 & 1.03 \\
    \midrule
    \textbf{전체 (27문항)} & \textbf{4.46} & \textbf{0.28} & \textbf{3.68} & \textbf{0.011} & \textbf{1.65} \\
    \bottomrule
  \end{tabular}
\end{table}
...
모델의 이론적 구조, 파라미터 설정, 실무 적용성, 산업적용 가능성 전반에 걸쳐
높은 수준의 긍정적 평가를 보인 것으로 해석된다.

앞서 \jrev{\subsecref{subsec:ch5_validity_reliability}}에서 확인한 바와 같이,
...
Lincoln과 Guba(\citeyear{lincoln1985naturalistic})의 4대 기준 즉, i) 신빙성(삼각 검증 및
참여자 확인), ii) 전이 가능성(풍부한 기술), iii) 의존 가능성(감사 추적 및 코딩 일관성),
iv) 확인 가능성(원자료 보관)을 통해 체계적으로 확보하였다.
...
리스크와 극단적 리스크를 분리하여 관리하는 이론적 근거를 제공한다''고 평가하였고, E3(리스크, 20년 이상)은
...
정성적 결과(\jrev{H10})가 동일한 결론을 \pdel{**}서로 다른 경로\pdel{**}로 도달하는
...
영역의 설문 점수가 상대적으로 낮은(4.67) 원인을 인터뷰가 ``통계적 승수 설명 필요''라는 맥락으로
...
삼각검증 수렴(4개 영역에서 확인 4건, 보완 4건, 불일치 0건)}의 다층적 검증 체계를 통해

...
파레토 C3 분석을 4.2(C2 가우시안 불확실성 결과)와 4.3(파레토 불확실성 환경의 C3 결과)에서 직접 수행하였다.
...
최대임을 보였다. 단계 4는 가우시안 불확실성 분석으로서 4.2(C2 가우시안 불확실성 결과)에 해당한다.
...
분석 및 C2--C3 교차비교로서 4.3(파레토 불확실성 환경의 C3 결과)과, 4.12(Lee and Cheung(2026)과의 교차 정합성)에 해당한다. C0$\to$C1$\to$C3 체계와 C2--C3 구조적
...
주요 검증 항목별로 정리한 것이다.
단계 1--3은 선행 연구인 \citep{lee2026drto}에 해당하고,
단계 4--5는 본 연구에서 수행한 분석이다.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{
    ...
    $0.52$배 감쇠 & --- & --- & --- & --- & \cmark &
      단계 5에서 발견·해명 \\
    \addlinespace
    순서 \m{준수율(C1$\leq$C$x$)} & \makecell{100\%\\(C1$\leq$C0)} & --- & --- & \makecell{99.87\%\\(C1$\leq$C2)} & \makecell{100\%\\(C1$\leq$C3)} &
      유형별 일관 \\
    \addlinespace
    전진 선택 & --- & --- & --- & $k_O$ & $U$ &
      지배 변수 전환 \\
...
\chg{둘째, 데이터 기반의 수치 정합을 보면,}
모든 연구 단계가 동일한 마스터 시드(42), 표본 크기($n = 10{,}000$),
기본 복구시간($R_0 = 6.0$h)을 사용하여
$\bar{\eta}_{\text{C1}} = 0.853$이 전 단계에서 일치한다.
\chg{\citet{lee2026drto}에서} $\kappa = 1.2$가 도출된 이후 본 연구의 \chg{C2와 C3} 분석에서
동일한 $\kappa$ 값이 적용되어, 연구 단계 간 파라미터 일관성이 보장된다.

\chg{셋째, 발견의 누적적 심화를 보면,}
본 연구의 C2 분석(단계 4)에서 이중 채널(관측성과 불확실성) 구조가 처음 정식화되고,
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{c >{\raggedright}p{4.5cm} >{\raggedright\arraybackslash}X}
    ...
    \midrule
    단계 1 & $\text{DRTO} \to 0$ \m{비현실적(선형 모델)} & 시그모이드 바운딩 $S(z) \in (-1,1)$ \\
    단계 2 & $\kappa$ \m{미결정(바운딩)} & 격자 탐색 $\to$ $\kappa^* = 1.2$ \\
    단계 3 & 단일 \m{채널($\kappa$ 최적화)} & 불확실성 채널 $E_{U,\text{bnd}}$ 추가 \\
    단계 4 & 경꼬리 \m{한계(C2 분석)} & 파레토 분포 \\
    단계 5 & 단일분포·\m{합성데이터(C3 분석)} & 통합 분석, 실무 적용, 전문가 검증 \\
    \bottomrule
  \end{tabularx}
\end{table}

5단계 연쇄가 \chg{\citet{lee2026drto}에서} 본 연구까지 단절 없이 연결되며,
...
본 연구의 \chg{C2와 C3} 분석에서 변경 없이 사용되며,
$R_{\max}$는 본 연구의 C2 분석(단계 4)에서 최초 도입($24.0$h $= 4R_0$)된 후
C3 분석(단계 5)에서 동일 값이 적용된다.
...
본 연구 전체에 대해 \chg{연구 모형과 본문, 온톨로지 분석, 기준값}(statistics.json)
간 \chg{수치와 수식 그리고 용어}의 정합성을 \m{교차검증}한 결과,
\textbf{총 303개 검증 항목이 모두 통과(PASS)}하였으며 \jrev{\tabref{tab:303_pass}}에 그 결과를 정리하였다.
...
\chg{가설과 조건} 정의 일치를 포함하며, \chg{\citet{lee2026drto}과} 본 연구 간의

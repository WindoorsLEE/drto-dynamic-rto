...
본 절에서는 본문 \subsecref{subsec:ch5_validity_reliability}에서 보고한
각 \chg{타당도와 신뢰도} 지표의 계산 공식과 산출 근거를 제시한다.

\chg{\textbf{내용타당도지수(CVI)}의 문항별 I-CVI는 5점 리커트 척도에서 4점 이상을 ``적합''으로 이분화하여 산출한다.}
\begin{equation}
...
\padd{\textbf{내용타당도비율(CVR)}은 본 연구에 적용하지 않았으나, 정의를 참조용으로 다음과 같이 제시한다.}
\begin{equation}
...
\chg{\textbf{Cronbach's $\alpha$}는 내적 일관성을 평가하며, 다음과 같이 산출한다.}
\begin{equation}
...
산출 결과 27개 문항 $\times$ 5명 평가자의 응답 행렬로부터
$\alpha = 0.89$\chg{로 산출되어 양호한 수준이다}.
...
\chg{\textbf{ICC(2,1)}~\citep{shrout1979icc}은 절대 일치(two-way random, single measures)를 평가하며, 다음과 같이 산출한다.}
\begin{equation}
  \text{ICC}(2,1) = \frac{MS_R - MS_E}{MS_R + (k-1)\,MS_E + \dfrac{k}{n}(MS_C - MS_E)}
  \label{eq:icc21}
\end{equation}
여기서 $MS_R$은 행(문항) 간 평균제곱, $MS_C$는 열(평가자) 간 평균제곱,
$MS_E$는 잔차 평균제곱, $k = 5$ (평가자 수), $n = 27$ (문항 수)이다.

산출 결과 ICC(2,1)은 $0.003$이다.
135개 응답 중 98.5\%가 4--5점에 집중하는 \textbf{천장효과(ceiling effect)}로 인해
$MS_C$와 $MS_E$의 차이가 극히 작아 ICC가 역설적으로 낮게 산출된다.
이는 전문가 간 불일치가 아닌 일관되게 높은 합의의 통계적 부산물이므로,
CVI(S-CVI/Ave $= 0.985$)를 주된 타당도 지표로 보고한다.

\chg{\textbf{ICC(3,1)}은 일관성(two-way mixed, single measures)을 평가하며, 다음과 같이 산출한다.}
\begin{equation}
...
\chg{\textbf{Krippendorff's $\alpha$}는 다음과 같이 산출한다.}
\begin{equation}
...
\]
여기서 $n$은 평가 대상 수({27} 문항), $k$는 평가자 수(5)이다.
...
\chg{\textbf{Fleiss' $\kappa$}~\citep{fleiss1971nominal}는 범주형 인터뷰 데이터에 대해 다음과 같이 산출한다.}
\begin{equation}
  \kappa = \frac{\bar{P} - \bar{P}_e}{1 - \bar{P}_e}
  \label{eq:fleiss}
\end{equation}
$\bar{P}$는 관측된 평균 일치율, $\bar{P}_e$는 우연에 의한 기대 일치율이다.
\begin{align}
...
산출 근거로, {12}개 하위 주제에서 만장일치(5/5 동의), {3}개 하위 주제에서 4/5 동의이므로 \chg{다음과 같이 계산된다.}
\begin{itemize}

...
4개 조건의 효율성 비율 $\eta = \DRTO / R_0$를 두 갈래 경로---가우시안
경로(C0$\to$C1$\to$C2)와 파레토 경로(C0$\to$C1$\to$C3)---로 구분하여
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ll rr cc l}
    ...
    $|d|$ & 해석 & \textbf{실무적 의미} \\
    \midrule
    \multicolumn{7}{l}{\textit{1단: 가우시안 경로 (C0 $\to$ C1 $\to$ C2)}} \\
    ...
    \midrule
    \multicolumn{7}{l}{\textit{2단: 파레토 경로 (C0 $\to$ C1 $\to$ C3)}} \\
    & C0 $\to$ C1 & $-0.147$ & $-1.180$ & $1.180$ & \jrev{매우 큰} & 관측성의 복구 단축 (동일) \\
...
%% --- 1단: 공통 단계 ---
\chg{먼저 C0에서 C1로의 관측성 복구 단축 효과는 양 경로에 공통으로 작용한다.}
...
즉 $\DRTO$가 기본 복구시간 $R_0$보다 단축됨을 의미한다.
...
이 점은 \jrev{\secref{sec:ch5_dual_channel}}에서 P85.8 교차점과
이중채널 감쇠 메커니즘을 통해 상세히 규명된다.
...
\textbf{셋째}, 순효과의 격차\chg{(}가우시안 $d = +1.110$ vs.\ 파레토 $d = +0.650$\chg{)}는

...
나타내며, 예를 들어 $\bar{F}(10) = 0.01$이면 불확실성이 10시간을 초과할 확률이 1\%임을 의미한다. 생존함수
값이 높을수록 해당 수준 이상의 극단 사건이 빈번함을 뜻하며, 분포의 꼬리가 얼마나 느리게 감소하는지를
직접 비교할 수 있다.}
두 분포는 동일한 기댓값 $E[U] = 3.0$을 공유하지만,
...
\begin{table}[htbp]
  \centering
  \caption{C3 조건의 인접 조건 대비 효과 \m{크기(Cohen's $d$)}}
  \label{tab:c3_effect_size}
  \begin{tabular}{lccl}
    \toprule
    비교 쌍 & $\Delta\bar{\eta}$ & Cohen's $d$ & 해석 \\
    \midrule
    C3 vs.\ C0 & $+0.514$ & $+0.650$ & 중간 효과 \\
    C3 vs.\ C1 & $\erf{+0.661}{+0.662}$ & $+1.169$ & 대효과(large effect) \\
...
비율(즉, 관측성만 적용한 C1의 복구 시간이 파레토 불확실성까지 반영한 C3 이하인 표본의 비율)은 $100.0\%$로서,
파레토 불확실성 조건에서는 C1 대비 C3의 순서가 \textit{예외 없이} 준수된다.
이는 파레토 분포의 높은 기댓값과 양의 스케일 특성에 의해,
...
조건부 꼬리 기댓값(Conditional Value-at-Risk, CVaR)~\citep{rockafellar2000optimization}은 특정 백분위 이상의
...
표본들의 조건부 평균이다. 예를 들어 $\mathrm{CVaR}_{99}$는 상위 1\%($n = 10{,}000$ 중 100개) 극단값의
...
\begin{table}[htbp]
  \centering
  \caption{C2 vs.\ C3 조건부 꼬리 기댓값(CVaR) 비교}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{C2 vs.\ C3 CVaR \m{비교(CVaR$_{80}$--CVaR$_{99}$)}}
  \label{fig:cvar_bar}
\end{figure}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{l rr l}
    ...
    순서 \m{준수율(C1$\leq$C$x$)} & 99.87\% & 100.0\% & 양쪽 모두 높음 \\
    \bottomrule
  \end{tabular}
\end{table}
회귀 분석 측면의 비교(1차/2차 $R^2$, 전진 선택, LASSO)는 4.4(회귀분석 결과)에서 상세히 다룬다.
...
본질적 우위를 보인다.
이러한 일상적 변동은 경꼬리 기반으로, 극단 위험은 중꼬리 기반으로 관리하는 ``이중축'' 구조는 5.3.2(이중채널 감쇠 기반 자원배분과 스트레스 테스트)에서 구체화된다.

...
본 연구에서는 C1, C2, C3 각 조건에 대해 3단계 \m{회귀 분석(1차 단순,
...
\chg{둘째, 1차 다중 회귀는} $\hat{\eta}_{\text{C1}}^{(1m)} = \erf{1.07 - 0.15\,k_O - 0.15\,O}{1.137 - 0.280\,k_O - 0.288\,O}$ ($R^2 = 0.869$)\chg{로서,}
관측성 원시 점수 $O$를 추가하면 설명력이 $86.9\%$로 두 배 이상
증가한다. $k_O$와 $O$의 회귀 계수 크기가 $\erf{-0.15}{-0.28}$로 거의 동일하여,
두 관측성 변수가 $\eta$ 감소에 동등하게 기여함을 보여준다.
\chg{셋째, \pdferr{2차 회귀}는 식 (4.1) 및 (4.2)에 나타낸 바와 같다.}
\begin{equation}
...
\chg{셋째, \pdferr{2차 회귀}는 식(4.3) 및 (4.4)에 나타낸 바와 같다.}
\begin{equation}
...
\begin{equation}
  \begin{aligned}
    = 1.082
    &+ 0.012\,k_O - 0.034\,O + 0.542\,U \\
    &- 0.307\,k_O^2 + 0.019\,O^2 - 0.013\,U^2 \\
    &- 0.545\,(k_O \cdot O) - 0.433\,(k_O \cdot U) + 0.001\,(O \cdot U)
  \end{aligned}
\end{equation}
\else
\begin{equation}
...
$k_O \cdot O$, $k_O \cdot U$ 등의 교호 작용이 핵심 구조를
이루며, C2 조건의 분산을 사실상 완전히 설명한다.
$k_O \cdot U$ 교호 작용의 존재는 관측성 가중치와 불확실성이
...
드러내는 것으로 해석된다.
\chg{셋째, 2차 회귀는 식 (4.5) 및 (4.6)에 나타낸 바와 같다.}
\begin{equation}
...
C1($1.000$) 및 C2($0.998$)와 달리 높은 설명력에는 도달하지 못한다.
이는 파레토 분포의 비선형성이 2차 회귀 다항식의 근사 능력을 초과함을 시사한다.

\chg{P85.8을 기준으로 분할 회귀를 수행하면 각 영역에서의 설명력이 현저히 개선된다.
Core 영역($\leq$ P85.8)에서는 \pdferr{2차 회귀} $R^2 = 0.999$로 C2와 유사한 완전 설명이 달성되며,
Tail 영역($>$ P85.8)에서도 분할 회귀 적용 시 \pdferr{2차 회귀} $R^2 = 0.710$으로 설명력이 개선된다.}

\p{[}표~\vref{tab:c3_split_regression}\p{]}는 C3 분할 회귀의 주요 결과를 요약한다.

\begin{table}[htbp]
  \centering
  \caption{C3 분할 회귀분석 \m{결과(P85.8 기준)}}
...
Core 영역($-0.798$) 대비 $0.52$배로 감쇠된다. 전체 계수는 공개 재현 패키지에 수록한다.
이 $0.52$배 감쇠가 \textbf{이중 채널 감쇠(dual-channel attenuation)}의
...
\chg{여기서 (1s)는 1차 단순, (1m)은 1차 다중, (2q)는 \pdferr{2차 회귀(교호 작용 포함)}를 가리킨다.}

\begin{table}[htbp]
  \centering
  \caption{조건별 회귀 분석 설명력($R^2$) 진행 비교}
  ...
  \begin{tabular}{lccc}
    ...
    C3 \m{전체(Pareto)} & 0.256 & 0.695 & 0.858 \\
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{조건별 $R^2$ 진행 \m{비교(1차 단순 $\to$ 1차 다중 $\to$ 2차 회귀)}}
  \label{fig:r2_progression}
\end{figure}

C1은 \pdferr{2차 회귀} 모형에서 $R^2 = 1.000$, C2는 $R^2 = 0.998$에 도달하여 개별 시그모이드 바운딩 함수가
\pdferr{2차 다항식}으로 높은 정밀도로 근사됨을 보여준다. C3는 전체 표본에서 $R^2 = 0.858$로
...
\p{[}그림~\vref{fig:regression_coefficients}\p{]}은 C1, C2, C3(전체) 1차 다중 회귀 모형의
주요 계수를 비교한 것이다.
1차 단순 회귀는 $k_O$ 단일 계수만 존재하여 조건 간 비교 차트의 의미가 제한적이고,
2차 회귀는 최대 9개 항의 계수가 포함되어 막대 차트보다 표 형태가
적합하므로 \p{[}표~\vref{tab:regression_progression}\p{]}를 참조하고,
3개 공통 변수($k_O$, $O$, $U$)를 갖는 1차 다중 회귀만 시각화한다.

\begin{figure}[htbp]
  \centering
  \ifdefined\ERRATAFIX\def\CoefCone{(절편, 1.14) ($k_O$, -0.28) ($O$, -0.29) ($U$, 0)}\else\def\CoefCone{(절편, 1.07) ($k_O$, -0.15) ($O$, -0.15) ($U$, 0)}\fi
\begin{tikzpicture}
    \begin{axis}[
      ybar,
      width=0.88\textwidth,
...
\p{[}표~\vref{tab:lasso_results}\p{]}은 정규화 계수 $\lambda$에 따른 생존 변수 수와
$R^2$를 요약한 것이다.
...
관측성-불확실성 교호 작용 중심의 핵심 구조가 확인된다.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{LASSO $R^2$ \m{경로(C2 vs.\ C3)}}
  \label{fig:lasso_path}
\end{figure}


\chg{이러한 두 방법의 교차검증 합의로 간명 모형을 도출한다.} 전진 선택법과 LASSO는 각각 단계별 최적(stepwise) 진입 순서와
...
합의 변수를 사용한 4변수 간명 모형의 회귀식은 \m{식~(\ref{eq:c2_parsimonious}) 및 (\ref{eq:c3_parsimonious})에 나타낸 바와 같다}\chg{.}
...
관측성과 불확실성의 교호 작용 구조가 세분화된 형태로 포착된다.
C3에서는 $k_O \cdot O$ 항이 선택되지 않아 $k_O$($-1.053$) 단일항에
관측성 효과가 집중되며, $U^2$ 항(계수 $-0.002$)이 포함되어
파레토 분포의 비선형 구조가 반영된다.
C3에서 $k_O$ 계수의 \m{절댓값}이 C2보다 큰 것은 관측성 영향력의 증대가
아니라, C2에서 교호작용항에 분산된 효과가 C3에서 단일항에
집중된 결과이다.
이는 \p{[}표~\vref{tab:c3_split_regression}\p{]}에서 보듯이 파레토 극단 영역에서 관측성의 한계적 효과가 감쇠하는
이중 채널 감쇠 현상($0.52$배 감쇠)과 일관된다.

\subsection{\mrev{회귀분석 가설 검증 결과}}
\label{subsec:ch4_h4_h5_conclusion}

\input{chapters/ch4/sec4.4_full_coeff}

...
\begin{table}[htbp]
  \centering
  \caption{\jrev{조건별 회귀분석 설명력($R^2$) 진행 종합 ($\kappa = 1.2$)}}
  ...
  \begin{tabular}{lccccc}
    ...
    C3 전체 (Pareto) & 10{,}000 & 0.256 & 0.695 & 0.858 & $+0.163$ \\
    \midrule
...
\chg{1차 단순 회귀($\eta$를 $k_O$에 대해 회귀)에서 결정계수 $R^2$는
C2($0.768$) $>$ C1($0.428$) $>$ C3($0.256$)의 순으로 나타난다. C2에서 $k_O$의
설명력이 가장 높은 것은 불확실성 도입이 관측성 가중치의 효과를
증폭시키기 때문이다.} \jrev{4.4절(회귀분석)}\chg{에서 보고하였듯이 $k_O$ 계수가
C1의 $-0.285$에서 C2의 $-1.878$로, 절대값 기준 약 $6.6$배 커졌다.
...
설명력은 C1($0.869$), C2($0.948$), C3($0.695$)으로 크게 향상된다.
...
%% -------------------------------------------------------------------
\subsection{\vrev{2차 회귀 모형의 필요성과 교호작용 효과}}
...
선형--약비선형 영역에 해당하여, 3차 이상의 고차 항의 기여가
수치 정밀도 수준에 불과하기 때문이다.
C1에서 핵심 교호작용항 $k_O \cdot O$(계수 $\approx -0.30$)를 포함하면
$R^2$가 $0.869 \to 1.000$으로 완전히 향상되며,
이 교호작용이 관측성 채널의 비선형 구조를 완벽히 포착한다.
C2에서는 $k_O \cdot O$에 더하여 $k_O \cdot U$(계수 $\approx -0.10$)
교호작용이 추가로 기여하며, 이 두 교호작용의 합이 $R^2$를
$0.948 \to 0.998$으로 향상시킨다.

\chg{다음으로 C3의 잔여 비설명 분산을 보면,}
...
\chg{둘째,} C1은 2차교호작용 모형에서 녹색($R^2 = 1.000$), C2는 $R^2 = 0.998$에 도달하여
개별 시그모이드 바운딩 함수의 높은 정밀도 근사가 확인된다.
\chg{셋째,} C3 전체의 2차 $R^2 = 0.858$는 녹색 기준($0.95$)에 미달하나,

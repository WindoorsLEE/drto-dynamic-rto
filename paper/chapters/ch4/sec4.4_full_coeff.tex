...
$O^2$ & $0.022$ & $0.5$ & 9 \\
$U^2$ & $-0.002$ & $-85.9$ & 3 \\
$k_O \cdot O$ & $-0.627$ & $\erf{-17.4}{-17.3}$ & 5 \\
$k_O \cdot U$ & $\erf{-0.126}{-0.125}$ & $-53.7$ & 4 \\
...
\midrule
\m{절편($\beta_0$)}      & \erf{1.000}{1.011}  & $<$0.001 & \erf{---}{7235.3}     & \erf{이론치 1.000}{이론치 1.000에 근접} \\
$k_O$ ($\beta_1$)     & \erf{0.000}{$-0.036$}  & $<$0.001 & \erf{---}{$-85.3$}     & \erf{주 효과 흡수됨}{주효과 소폭 잔존} \\
$O$ ($\beta_2$)       & \erf{0.000}{$-0.036$}  & $<$0.001 & \erf{---}{$-86.3$}     & \erf{주 효과 흡수됨}{주효과 소폭 잔존} \\
$k_O^2$ ($\beta_4$)   & \erf{0.000}{0.024}  & $<$0.001 & \erf{---}{64.4}     & \erf{비유의}{미세 곡률(유의)} \\
$O^2$ ($\beta_5$)     & \erf{0.000}{0.024}  & $<$0.001 & \erf{---}{65.2}     & \erf{비유의}{미세 곡률(유의)} \\
...
\erf{2차}{다중} 회귀의 전체 계수를 \p{[}표~\ref{tab:app-c3-tail-quad}\p{]}에 제시한다.
\erf{$k_O \cdot \ln U$ 교호작용이}{$k_O$ 주효과와 로그 변환 항 $\ln U$가} 꼬리 영역\erf{에서의
관측성 효과 변화}{의 변동}를 설명하는 핵심 항이다.

\begin{table}[htbp]
\centering
\caption{C3 Tail 영역 \erf{2차}{다중(lnU 변환)} 회귀 전체 계수 ($>$ P85.8)}
...
\midrule
절편          & \erf{0.50}{1.993}   & \\
$k_O$        & \erf{2.42}{$-1.664$}   & \erf{양의 주 \m{효과(교호작용과 결합)}}{음의 주효과(Core $-1.034$ 대비 확대)} \\
$\ln U$      & \erf{0.10}{0.824}   & 로그 변환된 불확실성\erf{}{(지배적)} \\
\erf{$k_O \cdot \ln U$}{$O$} & \erf{$-1.25$}{$-0.120$} & \erf{\textbf{핵심 \m{교호작용(증폭 원인)}}}{관측성 원시값(보조적)} \\
\midrule
$R^2$        & \erf{0.710}{0.860}  & \\
\bottomrule
\end{tabular}
...
\chg{\erf{\p{[}표~\ref{tab:app-c3-tail-quad}\p{]}에서 $k_O \cdot \ln U$ 교호작용($-1.25$)이 꼬리 영역에서 효과의 수학적 원인이며,
$\ln U$가 커질수록(극단 불확실성) $k_O$의 효과가 비선형적으로 변화한다.}{\p{[}표~\ref{tab:app-c3-tail-quad}\p{]}에서 $\ln U$ 계수($0.824$)가 꼬리 영역 변동의 지배적 원인이며, $k_O$ 계수($-1.664$)는 Core 영역($-1.034$) 대비 확대되어 극단 불확실성에서도 관측성 효과가 유지됨을 보여준다.}}

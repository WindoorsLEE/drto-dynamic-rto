...
등록·공개되었습니다. 등록본은 사후 수정이 불가능하기에, 확인된 모든 오류를 바로잡아
이 공개 페이지에 투명하게 밝혀 둡니다. 이용에 불편과 혼란을 끼쳐 드린 점 깊이 사과드립니다.

## 오류의 성격과 정정 원칙

이번에 확인·정정한 오류는 성격상 두 종류입니다.
...
- **개념의 혼동에서 비롯된 오류** — 서로 다른 개념의 값을 한 자리에 섞은 경우입니다.
  ...
  다른 공식으로 계산된 값이 혼입되어 있었습니다. 이런 오류는 값 자체가 겉으로는
  "그럴듯"하여, **정본 시뮬레이션 데이터로 모든 수치를 처음부터 재계산하고 개념
  정합성을 하나씩 대조**해야만 드러납니다. 이번 정오는 실제로 그 방식으로 전수 검증하였습니다.

**정정 원칙 — 사실은 바로잡되, 논지는 보존합니다.** 확인된 사실 오류(틀린 숫자·기호)는
정확한 값으로 바로잡되, 논문의 **주장·가설·결론은 그대로 유지**하였습니다. 예컨대
*F*(κ)를 논문 공식값으로 재계산해도 최적 곡률은 여전히 **κ\*=1.2**로 동일하며(오히려
봉우리가 더 뚜렷해집니다), 파레토가 가우시안 대비 꼬리가 훨씬 무겁다는(약 4배) 결론도
변하지 않습니다. 즉 이번 정오는 **수치를 정확히 바로잡은 것일 뿐, 논문의 어떤 결론도
뒤집지 않습니다.** 정정 위치·전후 값·근거는 아래 「오류 보고서」에 모두 정리되어 있습니다.
